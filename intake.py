...
class Intake_PDF:
    ...
    def load_pdf_file(self, file_path: str) -> list:
        ...
        for number in range(number_of_pages):
            ...
            intake_logger.debug(f"Page {number} text length: {len(text)}") # the length is in chars
            if len(text) < 100:
                intake_logger.warning(f"Page {number} text length is less than 100 chars. Text: {text}")
